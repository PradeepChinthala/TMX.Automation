...
<xsl:stylesheet version="1.0"
      ...
      <body>
        ...
        <xsl:variable name="summary">
          <xsl:call-template name="get-summary">
            <xsl:with-param name="dividedby" select="3600" />
          </xsl:call-template>
        </xsl:variable>
  ...
  <xsl:template name="summary-row">
    ...
    <td>
      <xsl:value-of select="msxsl:node-set($summary)/*/time"/>
    </td>
  </xsl:template>
  ...
  <xsl:template name="get-scenario-trace-mode">
    ...
    <xsl:choose>
      <xsl:when test="starts-with($text, '[IMG]')">TraceImage</xsl:when>
  ...
  <xsl:template name="scenario-line-output">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$traceMode = 'TraceImage'">
        <img width="100%" src="data:image/png;base64,{substring-after($text,'[IMG] ')}"/>
      </xsl:when>
      <xsl:otherwise>
        <xsl:value-of select="$text"/>
      </xsl:otherwise>
  ...
  <xsl:template name="get-summary">
    ...
    <xsl:param name="dividedby" select="60" />
    <testCaseSummary>
      <all>
      ...
      <time>
        <xsl:value-of select="format-number(sum($nodes[@executed = 'True']/@time) div $dividedby, '0.00')"/>
      </time>
    </testCaseSummary>
  </xsl:template>
  ...
  <xsl:template name="get-success-rate">
    ...
    <xsl:value-of select="format-number(msxsl:node-set($summary)/*/success div msxsl:node-set($summary)/*/all * 100,'#.00')"/>
    <xsl:text>%</xsl:text>
  </xsl:template>
